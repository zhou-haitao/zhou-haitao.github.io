...
class ReqMeta:
    ...
    def stage(self) -> SequenceStage:
        return SequenceStage.DECODE if self.num_output_tokens > 0 else SequenceStage.PREFILL
    @property
...
class ReqState():
    ...
    def retrieval(self, query: torch.Tensor, top_k: int):
        if top_k >= self.block_repre.shape[0]:
            ...
            block_ids = int(range(self.init_window_sz, n_blocks-self.local_window_sz+1))
            block_hashes = [f"{self.block_hash(self.req_meta.request_id, id_)}" for id_ in block_ids]
            return block_hashes
        ...
        topk_index = topk_index.sort().values # TODO: remove this, don't need to sort in decode
    ...
    def launch_transfer_task(self, transfer_type, block_hashes, vllm_block_ids):
        ...
        is_mla = False
        offsets_k = [get_offset(block_shape, self.rank, self.tp_size, precision, self.layer_id, is_v=False, is_mla=is_mla)] * length
        offsets_v = [get_offset(block_shape, self.rank, self.tp_size, precision, self.layer_id, is_v=True, is_mla=is_mla)] * length
    ...
    def save_blocks(self, num_blocks_need_dump):
        ...
        block_ids = list(range(num_blocks_dumped, num_blocks_dumped+num_blocks_need_dump))
        block_hashes = [f"{self.block_hash(self.req_meta.request_id, id_)}" for id_ in block_ids]
    ...
    def attention_begin(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        vllm_block_ids = self.req_meta.vllm_block_ids[self.init_window_sz:-self.local_window_sz]
...
class ESA(UcmSparseBase):
    ...
    def build_sparse_meta(self, scheduler_output: SchedulerOutput,
                          ...
                          input_batch: InputBatch) -> UcmSparseMetadata:
    ...
    def estimate_num_slots_sparsed(self, request: Request) -> int:
        ...
        mid_window_sz = int((num_blocks - INIT_WINDOW_SZ - LOCAL_WINDOW_SZ) * SPARSE_RATIO)
        flaw = request.num_tokens % block_size
        if flaw:
            flaw = block_size - flaw
        num_tokens_sparsed = (INIT_WINDOW_SZ + mid_window_sz + LOCAL_WINDOW_SZ) * block_size - flaw
